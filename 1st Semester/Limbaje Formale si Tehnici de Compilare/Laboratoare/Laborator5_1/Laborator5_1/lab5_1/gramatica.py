    ...
    def vfSecventa(self, secventaDeIntrare):
                ...
                if B == EPSILON:
                    for nr in self.follow[A]:
                        if tabelAnaliza[A][nr] and tabelAnaliza[A][nr] != (B, nrReguli[(A, B)]):
                            print(f"CONFLICT ({A}, {nr})") # daca ajungem pe o pozitie deja ocupata, avem conflict si nu putem aplica metoda de analiza sintactica
                            return
        ...
        while True: # verificam daca secventa este acceptata
            if tabelAnaliza[stiva[0]][bandaDeIntrare[0]] == "acc":
                ...
                stiva = stiva[1:]
            elif tabelAnaliza[stiva[0]][bandaDeIntrare[0]] == None: # daca am ajuns pe o celula a tabelului care e goala => eroare
