...
#pragma omp parallel for reduction(+:sum)   // reduction: iti permite sa aduni valorile variabilelor; ca un fel de GROUP BY pe SQL
    ...
    for (int i = 0; i < MAX; i++) {
        sum += a[i] * b[i]; // se creeaza o copie locala a sumei pt fiecare thread, care se va insuma dupa intreaga parcurgere
    }
...
int main()
{
    omp_set_num_threads(4); // setam nr. de thread-uri pe care, ulterior, le va crea OpenMP; Nu le creeaza!!

    cout << "Thread num1: " << omp_get_num_threads() << endl;
    // incepem cu 1 thread, la lini 73 avem 16 threaduri si la linia 104 isi fac join; threadurile nu sunt distruse!!! raman in thread pool, la 123 iar sunt mai multe threaduri
...
#pragma omp for schedule(static, 2) nowait  // schedule: modul in care vrem sa fie spart vectorul; schedule static face spargerea cum am facut pana acum la laburi (n/p)
        ...
        // static - distribuirea e la fel de fiecare data (in ordine)
        for (int i = 0; i < MAX; i++) {
            a[i] = omp_get_thread_num();
            b[i] = i; 
        } // cu nowait, ar merge mai departe si ar putea ajunge la calcularea sumei inainte ca vectorii sa fie initializati
...
#pragma omp for schedule(static, 2) collapse(2) // daca nu puneam collapse, paraleliza doar primul for (ar fi fost doar liniile distribuite intre threads);
        ...
        // face un fel de liniarizare a matricei si o imparte in bucati egale
        for (int i = 0; i < MAX; i++) {
            for (int j = 0; j < MAX; j++) {
...
#pragma omp sections    // sections: aici se vor da sectiunile ce vor fi executate
        {
#pragma omp section // cate un thread executa un section, deci avem 3 sections => 3 thread-uri folosite; 1 section = 1 thread
            {
                addVectors(a, b, c);
#pragma omp critical
                {
