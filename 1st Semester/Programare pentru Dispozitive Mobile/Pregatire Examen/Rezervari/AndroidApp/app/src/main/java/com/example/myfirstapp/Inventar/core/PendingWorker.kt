package com.example.myfirstapp.Inventar.core;
...
class PendingWorker(ctx:Context, params:WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        for(produs in appContext.container.database.produsDao().getAll()){
            if(!produs.isUpdated) {
                Log.d("PendingWorker", "REQUIRES CREATE ${produs}")
                appContext.container.itemRepository.quite_remove(produs)
                val res = appContext.container.itemRepository.save(produs)
                if(!res.isUpdated)
                    failed++
                else successful++
            }
