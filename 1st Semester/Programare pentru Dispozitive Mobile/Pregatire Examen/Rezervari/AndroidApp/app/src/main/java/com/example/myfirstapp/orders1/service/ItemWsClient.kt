...
class ItemWsClient(private val okHttpClient: OkHttpClient) {
    ...
    suspend fun openSocket(
        onEvent: (orderItem: OrderItem) -> Unit,
        onClosed: () -> Unit,
        onFailure: () -> Unit
    ) {
    ...
    inner class ItemWebSocketListener(
        private val onEvent: (orderItem: OrderItem) -> Unit,
        private val onClosed: () -> Unit,
        private val onFailure: () -> Unit
    ) : WebSocketListener() {
        private val moshi = Moshi.Builder().build()
        private val itemEventJsonAdapter: JsonAdapter<OrderItem> =
            moshi.adapter(OrderItem::class.java)
