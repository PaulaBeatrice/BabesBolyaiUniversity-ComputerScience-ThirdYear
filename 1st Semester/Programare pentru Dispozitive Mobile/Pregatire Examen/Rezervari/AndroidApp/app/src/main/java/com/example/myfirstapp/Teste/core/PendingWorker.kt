package com.example.myfirstapp.Teste.core;
...
class PendingWorker(ctx:Context, params:WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        for(question in appContext.container.database.questionDao().getAll()){
//            if(!question.isUpdated) {
                Log.d("PendingWorker", "REQUIRES UPDATE ${question}")
//                val res = appContext.container.itemRepository.update(question)
//                if(!res.isUpdated)
//                    failed++
//                else successful++
            }
