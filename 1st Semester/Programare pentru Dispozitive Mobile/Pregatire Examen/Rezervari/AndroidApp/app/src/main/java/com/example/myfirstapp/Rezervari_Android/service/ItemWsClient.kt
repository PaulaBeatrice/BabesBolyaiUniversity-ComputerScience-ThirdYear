...
class ItemWsClient(private val okHttpClient: OkHttpClient) {
    ...
    suspend fun openSocket(
        onEvent: (rezervare: Rezervare) -> Unit,
        onClosed: () -> Unit,
        onFailure: () -> Unit
    ) {
    ...
    inner class ItemWebSocketListener(
        private val onEvent: (rezervare: Rezervare) -> Unit,
        private val onClosed: () -> Unit,
        private val onFailure: () -> Unit
    ) : WebSocketListener() {
        private val moshi = Moshi.Builder().build()
        private val itemEventJsonAdapter: JsonAdapter<Rezervare> =
            moshi.adapter(Rezervare::class.java)
