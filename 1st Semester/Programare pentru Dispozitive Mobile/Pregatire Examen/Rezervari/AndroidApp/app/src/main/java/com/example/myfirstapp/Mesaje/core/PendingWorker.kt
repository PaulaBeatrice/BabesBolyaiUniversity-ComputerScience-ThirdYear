package com.example.myfirstapp.Mesaje.core;
...
class PendingWorker(ctx:Context, params:WorkerParameters) : CoroutineWorker(ctx, params) {
    ...
    override suspend fun doWork(): Result {
        ...
        for(mesaj in appContext.container.database.mesajDao().getAll()){
            if(!mesaj.isUpdated) {
                Log.d("PendingWorker", "REQUIRES UPDATE ${mesaj}")
                val res = appContext.container.itemRepository.update(mesaj)
                if(!res.isUpdated)
                    failed++
                else successful++
            }
