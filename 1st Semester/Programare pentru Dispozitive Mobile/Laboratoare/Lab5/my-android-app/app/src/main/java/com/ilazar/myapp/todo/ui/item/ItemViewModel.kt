...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveOrUpdateItem(date: String, sold: Boolean, price: Int, title: String, lat:Double, lng:Double) {
        viewModelScope.launch {
            Log.d(TAG, "saveOrUpdateItem...");
            try {
                uiState = uiState.copy(isSaving = true, savingError = null)
                val item = uiState.item?.copy(date=date, sold = sold, price = price, title = title, lat=lat, lng=lng)
                if (itemId == null) {
