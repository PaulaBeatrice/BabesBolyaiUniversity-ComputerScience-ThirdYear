...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveOrUpdateItem(productName: String,quantity: Int,date: String,checked: Boolean) {
        viewModelScope.launch {
            Log.d(TAG, "saveOrUpdateItem...");
            try {
                uiState = uiState.copy(submitResult = Result.Loading)
                val item = uiState.item.copy(title = productName,price = quantity,date = date, sold = checked)
                val savedItem: Item;
                if (itemId == null) {
