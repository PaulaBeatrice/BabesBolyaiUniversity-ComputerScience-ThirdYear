...
fun ItemScreen(itemId: String?, onClose: () -> Unit) {
    ...
    var sold by rememberSaveable { mutableStateOf(itemUiState.item.sold) }
    ...
    Scaffold(
        topBar = {
            TopAppBar(
                ...
                actions = {
                    Button(onClick = {
                        Log.d("ItemScreen", "save item text = $title, $price, $sold");
                        val date = Date().toString()
                        itemViewModel.saveOrUpdateItem(title,price,date,sold)
                    }) { Text("Save") }
                }
            )
        }
