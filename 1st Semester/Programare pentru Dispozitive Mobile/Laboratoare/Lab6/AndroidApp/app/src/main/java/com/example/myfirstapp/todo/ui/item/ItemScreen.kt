...
import com.example.myfirstapp.todo.ImagePicker
...
fun ItemScreen(itemId: String?, onClose: () -> Unit) {
    ...
    var price by rememberSaveable { mutableStateOf(itemUiState.game.price) }
    var isSold by rememberSaveable { mutableStateOf(itemUiState.game.isSold) }
    var date by rememberSaveable { mutableStateOf(convertStringToDate(itemUiState.game.date)) }
    var imageUrl by rememberSaveable { mutableStateOf(itemUiState.game.imageUrl) }
    ...
    LaunchedEffect(itemId, itemUiState.loadResult) {
        ...
        if (!(itemUiState.loadResult is Result.Loading)) {
            ...
            price = itemUiState.game.price
            isSold=itemUiState.game.isSold
            date= convertStringToDate(itemUiState.game.date)
            imageUrl = itemUiState.game.imageUrl
            ...
            TopAppBar(
                ...
                actions = {
                    ...
                    Button(onClick = {
                        ...
                        itemViewModel.UpdateItem(title, price, isSold, date, imageUrl)
