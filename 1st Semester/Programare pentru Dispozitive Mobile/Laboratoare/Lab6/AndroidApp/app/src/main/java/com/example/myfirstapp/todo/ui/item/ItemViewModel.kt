...
class ItemViewModel(private val itemId: String?, private val itemRepository: ItemRepository) :
    ...
    fun saveItem(title: String, price:Int, isSold: Boolean, date: Date, imageUrl:String){
        viewModelScope.launch {
            Log.d(TAG, "save new game!!!");
            try{
                uiState = uiState.copy(submitResult = Result.Loading)
                val item = uiState.game.copy(title=title, price = price, isSold = isSold
                    , date = convertDateToString(date), imageUrl = imageUrl)
    ...
    fun UpdateItem(title: String, price:Int, isSold: Boolean,date: Date, imageUrl:String) {
        viewModelScope.launch {
