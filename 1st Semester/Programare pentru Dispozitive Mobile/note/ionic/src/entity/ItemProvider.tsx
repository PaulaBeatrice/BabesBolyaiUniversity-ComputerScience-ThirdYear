...
export const ItemProvider: React.FC<PropsWithChildren> = ({ children }) => {
  ...
  async function syncLocalItems() {
    log("retrying to sync...");
    const { keys } = await Preferences.keys();
    for (let i = 0; i < keys.length; ++i) {
      const key = keys[i];
      if (key.startsWith("deleted_item")) {
        const preferencesKey = await Preferences.get({ key });
        const deletedId = preferencesKey.value;
        await deleteItemFn(Number(deletedId));
      }
    }
  }

  function syncLocalEffect() {
    const interval = setInterval(syncLocalItems, 10000);
  ...
  async function deleteItemFn(id: number) {
    try {
      ...
      const key = "deleted_item" + id;
      const preferencesKey = await Preferences.get({ key });
      if (preferencesKey.value) {
    ...
    closeWebSocket = newWebSocket((message) => {
      if (!canceled) {
        if (message.event == "deleted")
          dispatch({ type: DELETE_SUCCEEDED, payload: message.note });
        else dispatch({ type: SAVE_ITEM_SUCCEEDED, payload: message.note });
      }
    });
    return () => {
